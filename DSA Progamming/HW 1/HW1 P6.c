...
void maxHeapify(LinkNode **node, int index, int heapSize){
    int l = left(index);
    int r = right(index);
    int largest;
    if ((l < heapSize) && ((*node)->sorted[l] >= (*node)->sorted[index]))
        largest = l;
    else
        largest = index;
    if ((r < heapSize) && ((*node)->sorted[r] >= (*node)->sorted[largest]))
        largest = r;
    if (largest != index){
        int temp = (*node)->sorted[index];
        (*node)->sorted[index] = (*node)->sorted[largest];
        (*node)->sorted[largest] = temp;
        maxHeapify(node, largest, heapSize);
    }
    return;
}
void buildMaxheap(LinkNode **node){
    int len = (*node)->size;
    for (int i = 0; i < len; i++){
        (*node)->sorted[i] = (*node)->array[i];
    }
    for (int i = parent(len); i >= 0; i--)
        maxHeapify(node, i, len);
    return;
}
void heapSort(LinkNode **node){
    buildMaxheap(node);
    int heapSize = (*node)->size;
    for (int i = (*node)->size - 1; i > 0; i--){
        int temp = (*node)->sorted[i];
        (*node)->sorted[i] = (*node)->sorted[0];
        (*node)->sorted[0] = temp;
        heapSize--;
        maxHeapify(node, 0, heapSize);
    }
    return;
}
...
void reverse(int l, int r, LinkNode **head){
    ...
    if (node_l == node_r){
        ...
        /* adjust the order */
        LinkNode **node_list = (LinkNode **)malloc(340 * sizeof(LinkNode *));
...
int lessCount(int x, LinkNode *node){
    if ((node->size == 0) || (x <= node->sorted[0])){
        return 0;
    }
    if (x > node->sorted[node->size - 1]){
        return node->size;
    }
    int left = 0;
    int right = node->size;
...
void printNode(LinkNode *head){
    ...
    ptr = head;
    while (ptr != NULL){
        for (int i = 0; i < ptr->rev->size; i++)
            printf("%d ", ptr->rev->array[i]);
        printf("|");
        ptr = ptr->next;
    }
    printf("\n");
